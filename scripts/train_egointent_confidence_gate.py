...
class EgoIntentModel(nn.Module):
    ...
    def __init__(
        ...
        # Confidence-aware gate. Confidence is computed against a fixed
        # bank of TRAIN-set WHAT prototypes, so no GT WHAT is required
        # at validation/deployment.
        self.conf_gate = nn.Sequential(
            nn.Linear(fusion_dim + 1, 256),
            nn.Sigmoid(),
        )
    ...
    @staticmethod
    def what_confidence(predicted_what, what_bank):
        """
        Margin confidence against a fixed bank of normalized train-WHAT
        prototypes: top-1 cosine similarity minus top-2.
        """
        if what_bank is None or what_bank.ndim != 2:
            raise ValueError("what_bank must be a [K, D] tensor.")
        if what_bank.shape[0] < 2:
            raise ValueError("what_bank needs at least two prototypes.")

        sim = predicted_what @ what_bank.T
        top2 = torch.topk(sim, k=2, dim=-1).values
        confidence = (top2[:, :1] - top2[:, 1:2]).clamp(min=0.0)

        # Treat confidence as a routing signal, not another objective
        # for the WHAT predictor.
        return confidence.detach()

    def predict_confidence_guided(self, visual, what_bank):
        direct = self.predict_direct_why(visual)
        predicted_what = self.predict_what(visual)
        visual_state = self.visual_state(visual)
        ...
        confidence = self.what_confidence(
            predicted_what,
            what_bank,
        )
...
def build_what_prototype_bank(what_target, rows):
    """
    Build one normalized prototype per WHAT label using TRAIN data only.
    """
    groups = {}
    ...
    if len(prototypes) < 2:
        raise RuntimeError(
            "Need at least two unique WHAT labels for confidence routing."
        )
...
def evaluate(
    ...
    elif condition == "confidence_gate_guided":
        out = model.predict_confidence_guided(
            visual,
            what_bank,
        )
        what_pred = out["what"]
...
def train_one(
    ...
    what_bank = build_what_prototype_bank(
        train_what_target,
        train_rows,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if condition == "direct_why":
            ...
            if condition == "confidence_gate_guided":
                out = model.predict_confidence_guided(
                    train_visual,
                    what_bank,
                )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: confidence-aware routing of predicted-WHAT reasoning using train-set WHAT prototype margins."
        )
    )
    ...
    print("=" * 96)
    print("EGOINTENT CONFIDENCE-AWARE GATE DIAGNOSTIC")
    print("=" * 96)
    print("device:", device)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            ...
            torch.save(
                {
                    "experiment": "egointent_confidence_gate",
                    "condition": condition,
                    "seed": seed,
