...
class EgoIntentModel(nn.Module):
    ...
    def __init__(
        ...
        # Soft task-context branch used only by hierarchical_guided.
        # It predicts a compact task-context embedding from visual state
        # and predicted WHAT; no GT task is used at inference.
        ...
        self.hier_reasoner = nn.Sequential(
            nn.Linear(fusion_dim + 128, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, 256),
            nn.LayerNorm(256),
        )

        self.hier_gate = nn.Sequential(
            nn.Linear(fusion_dim + 128, 256),
            nn.Sigmoid(),
        )
    ...
    def predict_hierarchical_guided(self, visual):
        direct = self.predict_direct_why(visual)
        predicted_what = self.predict_what(visual)
        visual_state = self.visual_state(visual)
        ...
        task_ctx = F.normalize(
            self.task_context(base_fused),
            dim=-1,
        )

        hier_fused = torch.cat(
            [base_fused, task_ctx],
            dim=-1,
        )

        residual = F.normalize(
            self.hier_reasoner(hier_fused),
            dim=-1,
        )
        gate = self.hier_gate(hier_fused)
...
def train_one(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if condition == "direct_why":
            ...
            if condition == "hierarchical_guided":
                out = model.predict_hierarchical_guided(
                    train_visual
                )
            else:
            ...
            if condition == "hierarchical_guided":
                task_context_loss = multi_positive_infonce(
                    out["task_ctx"],
                    out["task_ctx"].detach(),
                    task_mask,
        ...
        total_loss = (
            ...
            + args.task_context_weight * task_context_loss
        )
        ...
        if (
            ...
            record = {
                ...
                "loss_task_context": float(task_context_loss.item()),
                "val": val_eval,
            }
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: hierarchical soft task-context conditioning for predicted-WHAT-guided WHY reasoning."
        )
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            "/media/dhqg/d1/vlia_outputs/"
            "egointent_hierarchical_fixed_v2"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Weight for soft task-context contrastive supervision.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Temperature for task-context contrastive loss.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT HIERARCHICAL TASK-CONTEXT GUIDED DIAGNOSTIC — FIXED")
    print("=" * 96)
    print("device:", device)
    ...
    print("task_context_weight:", args.task_context_weight)
    print("task_temperature:", args.task_temperature)
    print("=" * 96)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            ...
            torch.save(
                {
                    "experiment": "egointent_hierarchical_fixed",
                    "condition": condition,
                    "seed": seed,
