...
class EgoIntentModel(nn.Module):
    ...
    def predict_what_mixture_guided(
        self,
        visual,
        what_bank,
        mixture_k=3,
        mixture_temperature=0.07,
    ):
        """
        Use predicted WHAT only to retrieve the top-K TRAIN-set WHAT
        prototypes. A soft mixture of those prototypes conditions the
        WHY reasoner.

        No GT WHAT is used at validation/deployment.
        """
        direct = self.predict_direct_why(visual)
        predicted_what = self.predict_what(visual)
        visual_state = self.visual_state(visual)

        if what_bank is None or what_bank.ndim != 2:
            raise ValueError("what_bank must be [num_prototypes, dim].")

        k = min(int(mixture_k), int(what_bank.shape[0]))
        if k < 1:
            raise ValueError("mixture_k must be >= 1.")

        # Retrieval scores remain differentiable w.r.t. predicted_what.
        sim = predicted_what @ what_bank.T
        topk = torch.topk(sim, k=k, dim=-1)
        topk_scores = topk.values
        topk_idx = topk.indices

        selected = what_bank[topk_idx]  # [B, K, D]
        weights = F.softmax(
            topk_scores / mixture_temperature,
            dim=-1,
        )

        mixture_what = torch.sum(
            selected * weights.unsqueeze(-1),
            dim=1,
        )
        mixture_what = F.normalize(
            mixture_what,
            dim=-1,
        )

        fused = torch.cat(
            [visual_state, mixture_what],
            dim=-1,
        )
        ...
        return {
            "why": why,
            "direct": direct,
            # Keep raw predicted WHAT for the existing WHAT objective/metric.
            "what": predicted_what,
            "mixture_what": mixture_what,
...
def build_what_prototype_bank(what_target, rows):
    """
    One normalized target-space prototype per unique TRAIN WHAT label.
    """
    groups = {}
    for i, row in enumerate(rows):
        groups.setdefault(row["what"], []).append(i)
    ...
    for label in sorted(groups):
        idx = torch.tensor(
            groups[label],
            dtype=torch.long,
            device=what_target.device,
        )
        proto = what_target.index_select(0, idx).mean(dim=0)
        proto = F.normalize(proto, dim=0)
        prototypes.append(proto)
        labels.append(label)

    if not prototypes:
        raise RuntimeError("No WHAT prototypes were built.")

    return torch.stack(prototypes, dim=0), labels
...
def evaluate(
    ...
    elif condition == "what_mixture_guided":
        out = model.predict_what_mixture_guided(
            visual,
            what_bank,
            mixture_k=mixture_k,
...
def train_one(
    ...
    what_bank, what_bank_labels = build_what_prototype_bank(
        train_what_target,
        train_rows,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if condition == "direct_why":
            ...
            if condition == "what_mixture_guided":
                out = model.predict_what_mixture_guided(
                    train_visual,
                    what_bank,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: top-K mixture of train-set WHAT prototypes for ambiguity-aware WHY reasoning."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of top TRAIN WHAT prototypes mixed for WHY reasoning.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.07,
        help="Softmax temperature over top-K WHAT prototype similarities.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT TOP-K WHAT PROTOTYPE MIXTURE DIAGNOSTIC")
    print("=" * 96)
    print("device:", device)
    ...
    print("mixture_k:", args.mixture_k)
    print("mixture_temperature:", args.mixture_temperature)
    print("=" * 96)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            ...
            torch.save(
                {
                    "experiment": "egointent_what_mixture",
                    "condition": condition,
                    "seed": seed,
