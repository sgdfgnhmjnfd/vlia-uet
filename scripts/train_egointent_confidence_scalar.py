...
class EgoIntentModel(nn.Module):
    ...
    def what_confidence(predicted_what, what_bank):
        """
        Confidence = top-1 minus top-2 cosine similarity against
        TRAIN-set WHAT prototypes.
        """
        sim = predicted_what @ what_bank.T
        top2 = torch.topk(sim, k=2, dim=-1).values
        conf = (top2[:, :1] - top2[:, 1:2]).clamp(min=0.0)
        return conf.detach()

    def predict_confidence_scalar_guided(
        self,
        visual,
        what_bank,
        ...
        # Bounded scalar multiplier in [floor, 1].
        # sigmoid keeps the adjustment smooth and prevents gate collapse.
        conf01 = torch.sigmoid(
            confidence_scale * confidence
        )
        multiplier = (
            confidence_floor
            + (1.0 - confidence_floor) * conf01
        )

        gate = base_gate * multiplier
...
def build_what_prototype_bank(what_target, rows):
    ...
    if len(prototypes) < 2:
        raise RuntimeError("Need at least two WHAT prototypes.")
...
def evaluate(
    ...
    elif condition == "confidence_scalar_guided":
        out = model.predict_confidence_scalar_guided(
            visual,
            what_bank,
...
def train_one(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if condition == "direct_why":
            ...
            if condition == "confidence_scalar_guided":
                out = model.predict_confidence_scalar_guided(
                    train_visual,
                    what_bank,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: conservative confidence scalar modulation of the original learned intention gate."
        )
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            "/media/dhqg/d1/vlia_outputs/"
            "egointent_confidence_scalar_v1"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=10.0,
        help="Scale applied to WHAT top1-top2 confidence margin.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.75,
        help="Minimum multiplier applied to the original learned gate.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT CONFIDENCE-SCALAR GATE DIAGNOSTIC")
    print("=" * 96)
    print("device:", device)
    ...
    print("confidence_scale:", args.confidence_scale)
    print("confidence_floor:", args.confidence_floor)
    print("=" * 96)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            ...
            torch.save(
                {
                    "experiment": "egointent_confidence_scalar",
                    "condition": condition,
                    "seed": seed,
