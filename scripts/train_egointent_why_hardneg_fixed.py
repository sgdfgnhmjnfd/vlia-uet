...
# ============================================================
# WHY hard-negative objective
# ============================================================

def why_same_task_hard_negative_loss(
    pred,
    target,
    positive_mask,
    task_mask,
    ...
    """
    Same-task/different-WHY hard-negative margin loss.

    Positive: rows with the same WHY label.
    Hard negative: rows from the same task but a different WHY.
    """
    return same_task_hard_negative_loss(
        pred=pred,
        target=target,
...
def train_one(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        why_hard = torch.zeros(
            (),
            device=device,
        )
        if condition == "why_hardneg_guided":
            why_hard = why_same_task_hard_negative_loss(
                why_pred,
                train_why_target,
                why_mask,
                task_mask,
                margin=args.why_hard_margin,
            )

        total_loss = (
            why_loss
            + args.what_weight * what_loss
            + args.why_hard_weight * why_hard
        )
        ...
        if (
            ...
            record = {
                ...
                "loss_why_hard": float(why_hard.item()),
                "val": val_eval,
            }
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: direct same-task WHY hard-negative objective on top of the best predicted-WHAT-guided model."
        )
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            "/media/dhqg/d1/vlia_outputs/"
            "egointent_why_hardneg_fixed_v2"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.10,
        help=(
            "Weight of same-task/different-WHY hard-negative margin loss. "
            "Used only by why_hardneg_guided."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Cosine-similarity margin for WHY same-task hard negatives.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT WHY HARD-NEGATIVE GUIDED DIAGNOSTIC — FIXED GRADIENT")
    print("=" * 96)
    print("device:", device)
    ...
    print("why_hard_weight:", args.why_hard_weight)
    print("why_hard_margin:", args.why_hard_margin)
    print("=" * 96)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            ...
            torch.save(
                {
                    "experiment": "egointent_why_hardneg_fixed",
                    "condition": condition,
                    "seed": seed,
