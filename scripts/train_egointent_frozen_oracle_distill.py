...
def train_frozen_oracle_distill(
    seed,
    train_visual,
    train_what_target,
    train_why_target,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            teacher_why = teacher_out["why"].detach()
        ...
        if (
            ...
            val_eval = evaluate(
                ...
                why_target=val_why_target,
                rows=val_rows,
                condition="frozen_oracle_distill",
            )
            ...
            print(
                f"[frozen_oracle_distill] seed={seed} "
                f"epoch={epoch:03d}/{args.epochs} | "
                f"loss={total_loss.item():.6f} | "
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: frozen oracle-WHAT teacher distillation into deployable predicted-WHAT guided WHY."
        )
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            "/media/dhqg/d1/vlia_outputs/"
            "egointent_frozen_oracle_distill_v1"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Weight of frozen oracle-WHY teacher distillation.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT FROZEN ORACLE DISTILLATION")
    print("=" * 96)
    print("device:", device)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            if condition == "frozen_oracle_distill":
                result = train_frozen_oracle_distill(
                    seed=seed,
                    train_visual=train_visual,
                    train_what_target=train_what_target,
                    train_why_target=train_why_target,
            ...
            torch.save(
                {
                    "experiment": "egointent_frozen_oracle_distill",
                    "condition": condition,
                    "seed": seed,
