...
def soft_mrr_ranking_loss(
    pred,
    target,
    positive_mask,
    rank_temperature=0.05,
    positive_temperature=0.05,
):
    """
    Smooth surrogate for reciprocal rank.

    For each query:
      1) Aggregate all positive similarities into a smooth positive score.
      2) Estimate a soft rank:
           r = 1 + sum_j sigmoid((s_neg_j - s_pos) / tau)
      3) Minimize log(r).

    This uses the full candidate set, unlike a single hardest-negative
    margin objective, and directly favors moving positives toward the
    top of the ranking.
    """
    sim = pred @ target.T

    pos_logits = sim.masked_fill(
        ~positive_mask,
        float("-inf"),
    )

    # Smooth approximation to the strongest positive score.
    pos_score = (
        torch.logsumexp(
            pos_logits / positive_temperature,
            dim=1,
        )
    ...
    pairwise = torch.sigmoid(
        (sim - pos_score[:, None]) / rank_temperature
    )
    pairwise = pairwise * neg_mask.float()

    soft_rank = 1.0 + pairwise.sum(dim=1)

    # log-rank is monotonic with rank and numerically stable.
...
def train_one(
    ...
    if condition == "direct_why":
        ...
        # Same deployable architecture; softmrr_guided changes only
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        why_softmrr = torch.zeros(
            (),
            device=device,
        )
        if condition == "softmrr_guided":
            why_softmrr = soft_mrr_ranking_loss(
                why_pred,
                train_why_target,
                why_mask,
        ...
        total_loss = (
            ...
            + args.softmrr_weight * why_softmrr
        )
        ...
        if (
            ...
            record = {
                ...
                "loss_why_softmrr": float(why_softmrr.item()),
                "val": val_eval,
            }
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: full-list soft reciprocal-rank optimization for WHY on top of the best WHAT-hard-negative guided model."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Weight of the WHY soft reciprocal-rank surrogate.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Temperature for pairwise soft-rank comparisons.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Temperature for smooth aggregation of positive WHY scores.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT SOFT-MRR LISTWISE RANKING DIAGNOSTIC")
    print("=" * 96)
    print("device:", device)
    ...
    print("rank_positive_temperature:", args.rank_positive_temperature)
    print("=" * 96)
