...
def cosine_distill_loss(student, teacher):
    """Cosine distillation toward a frozen teacher representation."""
    return (1.0 - (student * teacher).sum(dim=-1)).mean()


def same_task_hard_negative_loss(
    pred,
    target,
    positive_mask,
    task_mask,
    ...
    """
    Margin loss specialized for WHAT.

    Positive: target rows with the same normalized WHAT label.
    Hard negative: target rows from the same task but with a different WHAT.

    For each query:
        max positive similarity should exceed the hardest same-task negative
        by at least `margin`.

    Rows with no valid same-task negative are ignored.
    """
    sim = pred @ target.T

    neg_mask = task_mask & (~positive_mask)

    pos_sim = sim.masked_fill(
        ~positive_mask,
        float("-inf"),
    ).max(dim=1).values

    neg_sim = sim.masked_fill(
        ~neg_mask,
        float("-inf"),
    ).max(dim=1).values

    valid = torch.isfinite(pos_sim) & torch.isfinite(neg_sim)
    ...
    return F.relu(
        margin + neg_sim[valid] - pos_sim[valid]
    ).mean()
...
def train_one(
    ...
    if condition == "direct_why":
        ...
        # Same architecture and trainable modules.
        # The only controlled difference is the WHAT objective.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if condition == "direct_why":
            ...
            if condition == "what_hardneg_guided":
                what_hard = same_task_hard_negative_loss(
                    what_pred,
                    train_what_target,
                    what_mask,
                    task_mask,
        ...
        if (
            ...
            record = {
                ...
                "loss_what_hard": float(
                    what_hard.item()
                    if "what_hard" in locals()
                    else 0.0
                ),
                "val": val_eval,
            }
...
def train_residual_oracle_distill(
    seed,
    train_visual,
    train_what_target,
    train_why_target,
    ...
    """
    Per-seed two-stage generalized distillation.

    Stage 1: train and select a genuine oracle_intention teacher.
    Stage 2: freeze that teacher and train a fresh predicted-WHAT student
    with the current hard-negative objective plus oracle-WHY distillation.

    GT WHAT is privileged training information only; student validation and
    deployment use predicted WHAT exactly as intention_guided does.

    Unlike full-WHY distillation, this version leaves the direct WHY branch
    untouched and distills only the residual produced by the intention path.
    """
    device = train_visual.device

    # ---------- Stage 1: train genuine oracle teacher ----------
    teacher_result = train_one(
        condition="oracle_intention",
        seed=seed,
    ...
    teacher = EgoIntentModel(
        hidden_dim=args.hidden_dim,
        visual_state_dim=args.visual_state_dim,
    ).to(device)
    teacher.load_state_dict(
        teacher_result["model_state_dict"]
    )
    teacher.eval()
    ...
    print(
        f"[frozen teacher] seed={seed} | "
        f"epoch={teacher_result['best_epoch']} | "
        f"oracle WHY MRR="
        f"{teacher_result['best_val']['why']['MRR']:.6f}"
    )

    # ---------- Stage 2: fresh deployable student ----------
    set_seed(seed)
    student = EgoIntentModel(
        hidden_dim=args.hidden_dim,
        visual_state_dim=args.visual_state_dim,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        out = student.predict_intention_guided(
            train_visual
        )
        why_pred = out["why"]
        what_pred = out["what"]
        student_residual = out["residual"]
        ...
        with torch.no_grad():
            teacher_out = teacher.predict_oracle_intention(
                train_visual,
                train_what_target,
            )
            teacher_residual = teacher_out["residual"].detach()

        # Distill only the intention-conditioned reasoning residual.
        # The direct WHY path remains governed by the normal WHY objective.
        distill_loss = cosine_distill_loss(
            student_residual,
            teacher_residual,
        )
        ...
        if (
            ...
            val_eval = evaluate(
                ...
                why_target=val_why_target,
                rows=val_rows,
                condition="residual_oracle_distill",
            )
            ...
            record = {
                ...
                "loss_why": float(why_loss.item()),
                "loss_what": float(what_loss.item()),
                "loss_what_hard": float(what_hard.item()),
            ...
            print(
                f"[residual_oracle_distill] seed={seed} "
                f"epoch={epoch:03d}/{args.epochs} | "
                f"loss={total_loss.item():.6f} | "
    ...
    if best_state is None:
        raise RuntimeError("No frozen-oracle-distill checkpoint selected.")
    ...
    return {
        ...
        "model_state_dict": best_state,
        "history": history,
        "teacher_best_epoch": teacher_result["best_epoch"],
        "teacher_best_val": teacher_result["best_val"],
    }
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "EgoIntent diagnostic: residual-only frozen oracle distillation into deployable predicted-WHAT guided WHY."
        )
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            "/media/dhqg/d1/vlia_outputs/"
            "egointent_residual_oracle_distill_v1"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.3,
        help=(
            "Weight of same-task/different-WHAT hard-negative margin loss. "
            "Used only by what_hardneg_guided."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Cosine-similarity margin for WHAT same-task hard negatives.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Weight of frozen oracle residual distillation.",
    )
    ...
    print("=" * 96)
    print("EGOINTENT RESIDUAL-ONLY ORACLE DISTILLATION")
    print("=" * 96)
    print("device:", device)
    ...
    print("what_hard_weight:", args.what_hard_weight)
    print("what_hard_margin:", args.what_hard_margin)
    print("distill_weight:", args.distill_weight)
    print("=" * 96)
    ...
    for condition in args.conditions:
        ...
        for seed in args.seeds:
            if condition == "residual_oracle_distill":
                result = train_residual_oracle_distill(
                    seed=seed,
                    train_visual=train_visual,
                    train_what_target=train_what_target,
                    train_why_target=train_why_target,
            ...
            torch.save(
                {
                    "experiment": "egointent_residual_oracle_distill",
                    "condition": condition,
                    "seed": seed,
